...
> **TypeScript 6 주의**: `baseUrl`이 TS6에서 deprecated. `tsconfig.app.json`에서 `baseUrl`을 제거하고 `paths`만 `"./src/*"` 패턴으로 사용한다.
>
> ```json
> // ❌ TS6에서 경고 발생
> "baseUrl": ".",
...
### 배열·문자열 메서드 호출 전 undefined 방어

`slice`, `map`, `filter`, `join` 등 배열·문자열 메서드를 호출할 때, 피연산자가 `null | undefined`일 수 있으면 반드시 폴백을 추가한다.

```tsx
// ❌ name이 undefined면 TypeError
...
API 응답 타입에 `string | null` 또는 `T[] | null`로 선언된 필드는 항상 이 패턴을 적용한다.
