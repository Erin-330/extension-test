...
## Spec Files (읽기 규칙)
- **기능명세서(이미지·문서)를 받아 새 페이지 MD를 작성할 때** → `.claude/spec-template.md` 먼저 읽고 그 구조에 따라 MD를 작성할 것. 작성 후 CLAUDE.md의 Spec Files 읽기 규칙에 새 항목을 추가할 것.
...
- `purchase-list.html` / 구매 내역 관련 작업 시 → `.claude/purchase-list.md` 먼저 읽을 것; **Rive 사용 필수** — 초기·추가 로딩 모두 `loading-page.riv`로 구현
