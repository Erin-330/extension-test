...
**아이콘 종류 불문 동일 규칙 적용** — 화살표, 섹션 헤더 아이콘, 메뉴 아이콘, 닫기 버튼, 등급 배지 등 페이지에 등장하는 **모든 아이콘**이 해당된다.
...
`▲`, `Λ`, `⚑`, `◁`, `›`, `✕`, `←`, `→`, `≡`, `•` 등 **모든 유니코드 문자와 HTML entity를 아이콘 대신 쓰는 것은 절대 금지**다.
화살표·방향·섹션 헤더·등급 아이콘은 특히 유니코드로 대체되기 쉬우므로 주의한다.
에셋 URL을 못 찾으면 자식 node-id로 추가 `get_design_context` 호출 — 어떤 경우에도 텍스트·문자 대체 불가.
