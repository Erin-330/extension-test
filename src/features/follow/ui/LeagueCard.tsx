...
export function LeagueCard({ item, selected, selectedNum, onClick }: Props) {
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      <div className="flex flex-1 gap-[12px] h-full items-center min-w-px px-[12px] relative">
        <div className="flex items-center p-[2px] rounded-[6px] shrink-0">
          <div className="overflow-hidden relative shrink-0 size-[28px]">
            <img
              src={item.asset}
              className={`absolute block max-w-none object-cover pointer-events-none size-full shrink-0 ${item.assetInset ?? ''}`}
              alt=""
            />
          </div>
        </div>
